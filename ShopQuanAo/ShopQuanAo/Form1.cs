...
namespace ShopQuanAo
{
    public partial class Form1 : Form
    {
        ...
        private void tmAd_Tick(object sender, EventArgs e)
        {
            if (isRed)
            {
                llbAd.BackColor = Color.Transparent; // Trở lại màu ban đầu
                llbAd.LinkColor = Color.Blue;
                lblKhauHieu.ForeColor = Color.Blue;
            }
            else
            {
                llbAd.BackColor = Color.Red; // Đổi sang màu đỏ
                llbAd.LinkColor = Color.Yellow; // Đổi sang màu đỏ
